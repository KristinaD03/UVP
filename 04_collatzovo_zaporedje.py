...
def najvecji_clen(n):
    if n == 1:
        return 1
    x = najvecji_clen(naslednji_clen(n))
    if x > n:
        return x
    if x < n:
        return n
...
def najdaljse_zaporedje(m, n):
   if n == m:
       return najvecji_clen(n)
   return najvecji_clen(dolzina_zaporedja(m), )
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NDYsInVzZXIiOjExNDI4fQ:1vxRRg:I61panVWAo_YVan534mWNpsGK4jcqOiOC86dFJX6hPA"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NDcsInVzZXIiOjExNDI4fQ:1vxRRg:hpn888K5BIITGEOc9mgrsmkqxX6mtLGZILCnlEZZpuE"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NDgsInVzZXIiOjExNDI4fQ:1vxRRg:zQa9ApHJJWjed_6GynVACV5N0LYPeHOTxjs1NVxVPos"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NDksInVzZXIiOjExNDI4fQ:1vxRRg:sum9_DOMTyv31mmkv2uv8cgHktltsf-cW1-bXBJOllg"
        try:
