...
def vsota_stevk_stevil_med(m, n):
    if n < m:
        return 0
    return vsota_stevk(n) + vsota_stevk_stevil_med(m, n-1)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NTYsInVzZXIiOjExNDI4fQ:1vxPru:WPQje2vfEwtjUaQPX4CgayGnOFOQdlUqHnqi5q2UmJU"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NTcsInVzZXIiOjExNDI4fQ:1vxPru:SCKi8gh3MlY7hfuYK7L_s9aYsCpVFSJHA1TcyPhyYww"
        try:
            try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NTgsInVzZXIiOjExNDI4fQ:1vxPru:FYP8XP5QmMPFwkaUateduom5fNOU0248i2jXNwAyOys"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NTksInVzZXIiOjExNDI4fQ:1vxPru:4WtPzEhMBKvpenR2lDMucyHUkGSLsQo8RDb5pYJb6KE"
        try:
            for i in range(1, 100):
