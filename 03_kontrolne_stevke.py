...
def vsota_kvadratov_stevk(n):
    vsota = sum(int(s) ** 2 for s in str(n))
    return vsota
...
def dodaj_kontrolno_stevko(sklic):
    vsota = sum(int(s) * (i + 2) for i, s in enumerate(str(sklic)[::-1]))
    ostanek = vsota % 11
    kontrolna_stevka = (11 - ostanek) % 11
    if kontrolna_stevka == 10:
        kontrolna_stevka = 0
    return sklic * 10 + kontrolna_stevka
print(dodaj_kontrolno_stevko(265195368523))
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjQsInVzZXIiOjExNDI4fQ:1vwHbZ:unHp8v1LlQpsHMazLL_tcLcStdoqOlO29RGKalavZqE"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjUsInVzZXIiOjExNDI4fQ:1vwHbZ:uxMHYs5Pp4lmuK7v3j2MM7WfcD89yThS_9VzUN6Br24"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjYsInVzZXIiOjExNDI4fQ:1vwHbZ:s1ZnDR-H_VtKBvY_8kSFCcKTIG1mQY5K-Vgd3L1stMA"
        try:
