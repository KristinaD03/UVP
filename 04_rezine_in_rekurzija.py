...
def filtriraj(n, m):
    if n == "":
        return ""
    if n[0].lower() in m.lower():
        return n[0] + filtriraj(n[1:], m)
    else:
        return "_" + filtriraj(n[1:], m)
...
def pretvori(s, b):
    ...
    if not s:
        return 0
    return znaki.index(s[-1].upper()) + b * pretvori(s[:-1], b)
# =====================================================================@027489=
# 3. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ4NiwidXNlciI6MTE0Mjh9:1w2RsB:-ULnv7nV-HxP0FH8SvdkV2rjX30Us9DywUm7DAcS_B8"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ5MCwidXNlciI6MTE0Mjh9:1w2RsB:guZFzrK8Xu0GN-JvSPmZhjXaXW8eN56-gQXVYMO8vTQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ4OSwidXNlciI6MTE0Mjh9:1w2RsB:-fa3MEyg4tipvk0c2kj_rDB9eH5cybZ174zLD_vmJzM"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ4NywidXNlciI6MTE0Mjh9:1w2RsB:kFj5F6UVPuisnRI6fuap3FI-U-HDBTlRLAivLfScxCM"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyNzQ4OCwidXNlciI6MTE0Mjh9:1w2RsB:TW8C-G39BOMnz2ZGySgca700ORslOV47CcVhYlRufk4"
        try:
